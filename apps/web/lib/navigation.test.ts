...
function flattenHrefs(role: string, options?: { showTeamsOps?: boolean; chatbotEnabled?: boolean }) {
...
assert.equal(flattenHrefs("DISPATCHER").includes("/chatbot"), true);
...
assert.equal(flattenHrefs("BILLING").includes("/chatbot"), true);
...
assert.equal(flattenHrefs("DISPATCHER", { chatbotEnabled: false }).includes("/chatbot"), false);
