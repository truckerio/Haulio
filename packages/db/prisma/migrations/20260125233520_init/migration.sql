-- DropForeignKey (guarded for environments before load confirmations exist)
ALTER TABLE IF EXISTS "LoadConfirmationExtractEvent" DROP CONSTRAINT IF EXISTS "LoadConfirmationExtractEvent_docId_fkey";

-- DropForeignKey (guarded for environments before tracking tables exist)
...
-- DropForeignKey (guarded for environments before operating entities exist)
ALTER TABLE IF EXISTS "OperatingEntity" DROP CONSTRAINT IF EXISTS "OperatingEntity_orgId_fkey";

-- DropForeignKey (guarded for environments before tracking integrations exist)
ALTER TABLE IF EXISTS "TrackingIntegration" DROP CONSTRAINT IF EXISTS "TrackingIntegration_orgId_fkey";

-- DropForeignKey (guarded for environments before telematics mapping exists)
...
-- DropForeignKey (guarded for environments before user invites exist)
...
-- AddForeignKey (guarded for environments before user invites exist)
...
-- AddForeignKey (guarded for environments before operating entities exist)
...
-- AddForeignKey (guarded for environments before tracking integrations exist)
...
-- AddForeignKey (guarded for environments before telematics mapping exists)
...
-- AddForeignKey (guarded for environments before tracking tables exist)
...
-- AddForeignKey (guarded for environments before load confirmations exist)
